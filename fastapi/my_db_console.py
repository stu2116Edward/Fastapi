...
# 服务器地址,用户名,密码,数据库名
db_manager = DatabaseManager('you server', 'user_name', 'password', 'sql_name')
# 新建表
# db_manager.create_table('test', [
...
# db_manager.insert_data('test', ['user_name', 'phone', 'age', 'email', 'pwd'], ['test', '999', 18, 'test@gmail.com', '888666'])

# 删除记录
# db_manager.delete_record('test', "user_name = 'test'")

# 添加新字段
# db_manager.add_column('test', 'new_column', 'VARCHAR(20)')

# 删除字段
# db_manager.drop_column('test', 'new_column')

# 删除表
# db_manager.drop_table('tb_27_hhr_users')

# 查询数据示例
# 查询表中所有记录
db_manager.select_data('test')
# 根据条件查询指定列
# db_manager.select_data('test', columns=['user_name', 'phone', 'age', 'email', 'pwd'], condition="user_name='Edward'")
